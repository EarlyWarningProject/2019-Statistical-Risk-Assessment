...
```{r, eval = FALSE}
# comparing battledeaths variable using old and new subset criteria
sub <- dat[, c("country_name", "year", "battledeaths1","battledeaths_new")]
sub$diff <- sub$battledeaths1 - sub$battledeaths_new
summary(sub$diff)
library(ggplot2)


sub$ln.bd1 <- log(sub$battledeaths1 +1)
sub$ln.bdnew <- log(sub$battledeaths_new +1)
ggplot(sub) + geom_point(aes(x = battledeaths_new, y = battledeaths1)) + 
  geom_abline(slope = 1, intercept = 0, color = "red")
cor(sub$ln.bd1, sub$ln.bdnew)
```
...
# check
# compare <- merge(dat_new[dat_new$year == carry_from+1, 
#                          c("sftgcode", "mkl.ongoing")], 
#                  dat[dat$year == carry_from, c("sftgcode", "mkl.ongoing")],
#                  by = "sftgcode")
# all.equal(compare$mkl.ongoing.x, compare$mkl.ongoing.y)
# all.equal(compare$mkl.ever.x, compare$mkl.ever.y)
```
...
# new_country.mk <- sapply(end$new_sftg, function(x) dat$year[dat$sftgcode == x & dat$anymk.start == 1])
# names(new_country.mk) <- end$country_name
# 
# earliest_mk <- lapply(new_country.mk, function(x) min(x))
# earliest_mk <- data.frame(unlist(earliest_mk), names(earliest_mk))
# end <- merge(end, earliest_mk, by.x = "country_name", 
#              by.y = "names.earliest_mk.", all.x = TRUE)
# end$diff <- end$unlist.earliest_mk. - end$last_year
```
...
```{r}
dat$country_name[dat$mkl.ongoing & dat$year == 2017]
# should congo be here?
```
